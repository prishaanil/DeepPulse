...
class PatchTST(nn.Module):
    ...
    def __init__(self, c_in:int, target_dim:int, patch_len:int, stride:int, num_patch:int, 
        ...
        self.shared_embedding = shared_embedding  # Initialize shared_embedding as an attribute
        self.head_type = head_type
        self.W_pos = nn.Parameter(torch.zeros(1, num_patch, d_model))
    ...
    def forward(self, x) -> Tensor:
        ...
        if not self.shared_embedding:
            ...
            for i in range(n_vars): 
                z = self.W_P[i](x[:, :, i, :])  # Project each variable independently
                #print(f"[DEBUG] After projecting variable {i}, z shape: {z.shape}")
                x_out.append(z)
            x = torch.stack(x_out, dim=2)  # Stack along the variable dimension
            #print(f"[DEBUG] After stacking, x shape: {x.shape}")
        else:
            x = self.W_P(x)  # Shared projection for all variables
            #print(f"[DEBUG] After shared projection, x shape: {x.shape}")

        x = x.transpose(1, 2)  # [bs, nvars, num_patch, d_model]
        #print(f"[DEBUG] After transpose, x shape: {x.shape}")

        # Reshape to [bs * nvars, num_patch, d_model]
        u = torch.reshape(x, (bs * n_vars, num_patch, self.d_model))
        #print(f"[DEBUG] After reshape to [bs * nvars, num_patch, d_model], u shape: {u.shape}")

        # Dynamically resize self.W_pos if necessary
        if self.W_pos.shape[1] != num_patch:
            self.W_pos = nn.Parameter(torch.zeros(1, num_patch, self.d_model, device=u.device))  # Resize W_pos to match num_patch and move to the correct device
            #print(f"[DEBUG] Resized W_pos to: {self.W_pos.shape}")

        # Add positional encoding
        u = self.dropout(u + self.W_pos)  # [bs * nvars, num_patch, d_model]
        #print(f"[DEBUG] After adding positional encoding, u shape: {u.shape}")

        # Encoder
        z = self.backbone.encoder(u)  # Use the encoder from the backbone
        #print(f"[DEBUG] After encoder, z shape: {z.shape}")

        # Reshape back to [bs, nvars, num_patch, d_model]
        z = torch.reshape(z, (-1, n_vars, num_patch, self.d_model))
        #print(f"[DEBUG] After reshape back to [bs, nvars, num_patch, d_model], z shape: {z.shape}")

        # Permute to [bs, nvars, d_model, num_patch]
        z = z.permute(0, 1, 3, 2)
        #print(f"[DEBUG] After permute to [bs, nvars, d_model, num_patch], z shape: {z.shape}")
...
class ClassificationHead(nn.Module):
    ...
    def forward(self, x):
        ...
        """
        # x: [bs x nvars x d_model x num_patch]
        x = x.mean(dim=-1)  # mean over patches
        x = self.flatten(x)
        x = self.dropout(x)
        y = self.linear(x)
        return y
